    ...
    private static String dotToBig(String dotPart) {
        ...
        for (int i = 0; i < dotPart.length() && i < 3; i++) {
            ...
            if ((num = Integer.parseInt(dotPart.substring(i, i + 1))) != 0)
                sb.append(NUM.substring(num, num + 1)).append(DOTUNIT.substring(i, i + 1));
        }
        return sb.toString();
    }
    ...
    private static String getSubUnit(String strVal) {
        ...
        for (int i = 0; i < strVal.length(); i++) {
            ...
            if (num == 0) {
                ...
                // “壹拾”合理
                // Else
                sb.append(NUM.substring(num, num + 1));
                // End If
                // 追加单位
                if (i != strVal.length() - 1)// 个位不加单位
                    sb.append(UNIT.substring(i + 4 - strVal.length(), i + 4 - strVal.length() + 1));
            }
        }
        return sb.toString();
    }
