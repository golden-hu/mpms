...
public class ImageUtil {
     ...
     *
     * @param inFilePath
     *            要截取文件的路径, 输出文件为filename_t.ext(文件名中间加_t)
     * @param width
     *            要截取宽度
     * @param hight
     *            要截取的高度
     ...
     * @throws Exception
     */
    public static void saveThumbnail(String inFilePath, int width, int hight, boolean... proportion) {
        saveThumbnail(inFilePath, FileUtil.getThumbnailName(inFilePath), width, hight, proportion);
    }
    /**
     * 实现图像的等比缩放
     *
     ...
     * @throws Exception
     */
    public static void saveThumbnail(String inFilePath, String outFilePath, int width, int hight,
                                     boolean... proportion) {
        boolean isProp = true;
        ...
        try {
            ...
            if (width > 0 || hight > 0) {
                ...
                if (sw > width && sh > hight) {
                    srcImage = resize(srcImage, width, hight, isProp);
                }
    ...
    public static void saveSubImage(String src, String dest, int x, int y, int width, int height) {
        ...
        try {
            ...
            if(width + x > image.getWidth()){
                width = image.getWidth() - x;
            }
            if(height + y > image.getHeight()){
                height = image.getHeight() - x;
            }
